...
use super::*;
use crate::publish::address::{Address, Bucket, Report, Resolution, Scope, Window};
...
fn every_document_carries_the_indexed_tags_and_a_human_readable_alt() {
    // Arrange / Act
    let tags = tags(&window_address(), &run(), 1, None);
...
fn a_series_partition_also_names_its_resolution_and_period() {
    let period = Period {
        from: 1_767_225_600,  // 2026-01-01T00:00:00Z
        until: 1_769_904_000, // 2026-02-01T00:00:00Z
    };

    let tags = tags(&series_address(), &run(), 3, Some(period));
...
fn the_alt_of_a_series_partition_reads_as_one_sentence() {
    let tags = tags(&series_address(), &run(), 1, None);
...
fn the_index_is_described_as_the_index() {
    let tags = tags(&Address::Index { year: None }, &run(), 1, None);
    let alt = find(&tags, "alt")[0];
...
fn every_report_and_window_has_words_of_its_own_in_the_alt() {
    ...
    let alt_of = |address: &Address| find(&tags(address, &run(), 1, None), "alt")[0].to_string();
...
fn the_envelope_wraps_a_payload_with_the_run_and_the_revision() {
    let envelope = Envelope::new(&run(), 2, serde_json::json!({"range": {}, "metrics": []}));
...
fn a_restated_envelope_says_when_and_why() {
    let envelope = Envelope::new(&run(), 2, serde_json::json!({})).restated(
        1_787_800_000,
        "the rate book gained a snapshot that priced three orders",
    );
    let json = serde_json::to_value(&envelope).expect("serialises");
    ...
    // And one that was not restated carries neither field at all: an
    // absent restatement is not a restatement with empty reasons.
    let plain =
        serde_json::to_value(Envelope::new(&run(), 1, serde_json::json!({}))).expect("serialises");
