...
fn without_the_assumption_the_inferred_rows_are_not_offered() {
    ...
    assert!(
        !catalogue(&data(), NOW).contains(&"dev_fees.implied_volume".to_string()),
    ...
    assert!(catalogue(&with_assumption, NOW).contains(&"dev_fees.implied_volume".to_string()));
}

#[test]
fn the_catalogue_is_what_the_families_report_minus_what_cannot_be_plotted() {
    let catalogue = catalogue(&data(), NOW);
