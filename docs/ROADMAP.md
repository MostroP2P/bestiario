...
| 47 | `feat(publish): a snapshot in one pass` | L | 46 | Every document of a snapshot computed from one read of the archive, sharing one `snapshot_id` (§7): window documents carrying the SPEC §10 envelope verbatim, series partitions in the columnar form of §6.2. A series partition is a series, so it goes through the aggregation of PR 42 rather than a second implementation — computing it twice would be two answers under one heading. Absence follows §6.3: a bucket outside coverage is `null` in every column, counts included, which is the rule the daily reports already apply and the single most misleading thing this feature could get wrong. |
| 48 | `feat(publish): the index` | M | 47 | `d = index` (§5): what exists, what changed and what is current. Content hash per document, `revision`, `coverage` from the archive's real extent, and the `resolutions` a client picks from. Written to be shardable by year later without a client change (§5.1). |
...
| 51 | `feat(publish): restatement and --republish` | M | 50 | `revision`, `restated_at` and `restated_because` (§8), and the rule that an unchanged hash is not a revision and is not re-signed. `--republish [--from] [--until]` regenerates a range from the archive alone, which is what makes "the relay is not the archive" (§1.1) true rather than aspirational: the recovery path for a pruned relay, a new relay or a schema migration. |
